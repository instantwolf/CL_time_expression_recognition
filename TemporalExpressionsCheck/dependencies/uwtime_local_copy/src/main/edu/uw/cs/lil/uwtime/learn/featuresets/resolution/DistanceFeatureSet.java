...
public class DistanceFeatureSet extends TemporalResolutionFeatureSet {
	private static final int[]	BINS				= { -200, -100, -32, -8, 0,
			1, 8, 32, 100, 200						};
	private static final int	MINIMUM_GRANULARITY	= new TemporalYear(0)
															.getGranularity();
	...
	private void setDistanceFeature(IHashVector feats, LocalDateTime from,
			LocalDateTime to, String label) {
		try {
			final int delta = Days.daysBetween(from, to).getDays();
	...
	protected IHashVectorImmutable setMentionFeats(MentionResult result,
		...
		if (result.getEntity() instanceof TemporalSequence
				&& result.getContext().getDirection() != TemporalContext.TemporalDirection.NONE) {
			final TemporalSequence sequence = ((TemporalSequence) result
					.getEntity()).normalize();
			if (sequence.isRange()) {
				if (sequence.getDeepestNode().getGranularity() > MINIMUM_GRANULARITY) {
					setDistanceFeature(feats, result.getContext()
							.getReference().getStartJodaTime(),
							sequence.getStartJodaTime(), "FROM_REF");
				}
			} else {
				feats.set(new KeyArgs(getFeatureTag(), "NOT_FULLY_SPECIFIED"),
						1);
			}
		}
		return feats;
