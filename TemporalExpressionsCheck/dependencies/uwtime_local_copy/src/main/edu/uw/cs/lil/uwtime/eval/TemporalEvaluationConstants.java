...
public class TemporalEvaluationConstants {
	...
	private void addAllConstants() {
		...
		addConstant("timeofday:s", new TemporalSequence(
				new TemporalTimeOfDay(0)));
		...
		addConstant("present_ref:a", new TemporalApproximateReference(
				"PRESENT_REF"));
		addConstant("future_ref:a", new TemporalApproximateReference(
				"FUTURE_REF"));
	...
	private void addAllPredicates() {
		...
		addPredicate("every:<s,s>", new TemporalQuantify(
				TemporalQuantifier.EVERY));
		addPredicate("some:<d,d>",
				new TemporalQuantify(TemporalQuantifier.SOME));
		...
		addPredicate("less_e:<d,d>",
				new TemporalModify(TemporalModifier.LESS_E));
		addPredicate("more_e:<d,d>",
				new TemporalModify(TemporalModifier.MORE_E));
		...
		addPredicate("approx:<d,d>",
				new TemporalModify(TemporalModifier.APPROX));
		addPredicate("approx:<s,s>",
				new TemporalModify(TemporalModifier.APPROX));
	...
	private void addConstant(String semantics, Object constant) {
		constantObjects.put(LogicalConstant.read(semantics), constant);
	}
	...
	private void addPredicate(String semantics, TemporalPredicate predicate) {
		 ...
		 * while (e instanceof Lambda) e = ((Lambda) e).getBody();
		 * predicates.put((LogicalConstant)((Literal) e).getPredicate(),
		 * predicate);
		 */
		predicates.put(LogicalConstant.read(semantics), predicate);
	}
}
