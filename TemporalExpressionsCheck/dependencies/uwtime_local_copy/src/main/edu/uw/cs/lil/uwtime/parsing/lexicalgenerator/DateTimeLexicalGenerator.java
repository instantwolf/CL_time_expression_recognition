 ...
 * @author Kenton Lee
 */
public class DateTimeLexicalGenerator implements
	...
	public DateTimeLexicalGenerator(
	...
	public Set<LexicalEntry<LogicalExpression>> generateLexicon(
		...
		final Set<LexicalEntry<LogicalExpression>> lexicalEntries = new HashSet<>();
		for (final String token : evidence.getTokens()) {
			for (final DateTimeParser p : formats) {
				final LexicalEntry<LogicalExpression> entry = p
						.getLexicalEntry(token);
				if (entry != null) {
	...
	private abstract class DateTimeParser {
		...
		public LexicalEntry<LogicalExpression> getLexicalEntry(String token) {
			...
			if (semantics != null) {
				return new LexicalEntry<>(Collections.singletonList(token),
						categoryServices.parse("NP : " + semantics),
						DATETIME_LEXICAL_ORIGIN);
			} else {
				return null;
			}
		}
	...
	private class HMParser extends DateTimeParser {
		...
		String getSemantics(String token) throws ParseException {
			final LocalDateTime joda = new LocalDateTime(format.parse(token));
			return intersect(nth(joda.getHourOfDay() + 1, "hour"),
	...
	private class HMSParser extends DateTimeParser {
		...
		String getSemantics(String token) throws ParseException {
			final LocalDateTime joda = new LocalDateTime(format.parse(token));
			return intersect(
					intersect(nth(joda.getHourOfDay() + 1, "hour"),
							nth(joda.getMinuteOfHour() + 1, "minute")),
	...
	private class YMDParser extends DateTimeParser {
		...
		String getSemantics(String token) throws ParseException {
			final LocalDateTime joda = new LocalDateTime(format.parse(token));
			return intersect(
					intersect(nth(joda.getYear(), "year"),
