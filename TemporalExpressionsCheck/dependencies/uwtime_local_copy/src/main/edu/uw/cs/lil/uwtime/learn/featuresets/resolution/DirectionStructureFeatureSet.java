...
public class DirectionStructureFeatureSet extends TemporalResolutionFeatureSet {
	...
	protected IHashVectorImmutable setMentionFeats(MentionResult result,
		...
		feats.set(new KeyArgs(getFeatureTag(), "ref="
				+ getStructure(result.getContext().getReference()), "result="
				+ getStructure(result.getEntity()), result.getContext()
				.getDirection().toString()), 1);
		return feats;
	}
}
