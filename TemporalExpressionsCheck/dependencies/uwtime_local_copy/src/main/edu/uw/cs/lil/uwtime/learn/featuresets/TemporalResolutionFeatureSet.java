...
public abstract class TemporalResolutionFeatureSet
		implements
		IJointFeatureSet<ISituatedDataItem<Sentence, TemporalJointChunk>, MentionResult> {
	...
	public List<Triplet<KeyArgs, Double, String>> getFeatureWeights(
			IHashVector theta) {
		final List<Triplet<KeyArgs, Double, String>> weights = new LinkedList<>();
		for (final Pair<KeyArgs, Double> feature : theta
				.getAll(getFeatureTag())) {
	...
	public double score(MentionResult executionStep, IHashVector theta,
		...
		return setMentionFeats(executionStep, HashVectorFactory.create(),
				dataItem.getState()).vectorMultiply(theta);
	}
	...
	protected abstract IHashVectorImmutable setMentionFeats(
			MentionResult executionStep, IHashVector feats,
